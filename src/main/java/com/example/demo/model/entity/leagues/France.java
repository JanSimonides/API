...
public class France {
    ...
    public int score (){
        ...
        String[] result  = {"null","PSG", "Lyon", "Marseille", "Rennes", "Nice", "Strasbourg", "Angers", "Bordeaux", "Montpelier",
                "Nantes", "Lille", "Lorient", "Monaco", "St Etienne", "Nimes", "Reims", "Lens", "Metz", "Brest", "Dijon"};
        ...
        for (i=1; i<=20;i++){
            for (j=1; j<=20;j++){
                if (array[i].equals(result[j])){
                    //System.out.println(array[i]+" "+result[j]+ " za "+Math.abs(i-j)+" bodov");
                    points +=  Math.abs(i-j);
                }
            }
        }
